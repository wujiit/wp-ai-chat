...
全开源免费版 - WordPress ai助手插件，可实现：ai对话、ai文章生成、ai文章总结、ai文章翻译、文章内容朗读、ai回复内容朗读。  
这个免费版本采用非流式实现的ai对话，如果ai返回的内容过多，可能会卡，无法及时显示出来。（只是对话聊天不一样，其他都一样）  
文章翻译和语音朗读原本是另外一个单独插件的，是之后才合并进来的。
